...
/*            request.addEventListener('load', () => {            // отслеживаем загрузку запроса
    ...
    showSlides(slideIndex);                             // инициализация картинки
    ...
    function showSlides(n) {                            // делаем переход между граничными слайдами
        if (n > slides.length) {
            slideIndex = 1;
        }
        ...
        slides[slideIndex - 1].style.display = 'block';         // отображаем тот, чья очередь
    ...
    prev.addEventListener('click', () => {               // обработчики событий кнопок
        plusSlides(-1);
    });
